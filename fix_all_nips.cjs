...
async function run() {
  ...
  // Actually we don't have listUsers without service_role key...
  // But wait! `profiles` table has `full_name`. Can we just reset the whole database?
  console.log("We can't easily get emails without service role. Let's output what we have.");
}
run();
