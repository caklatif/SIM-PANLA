...
const OperatorDashboard: React.FC = () => {
  ...
  const canAccessGroupAttendance =
    isAdmin ||
    isOperator ||
    profile?.role === "admin" ||
  ...
  const [studentClassCounts, setStudentClassCounts] = useState<
    Record<string, number>
  >({});
  const [absenceStats, setAbsenceStats] = useState({ S: 0, I: 0, A: 0, D: 0 });
  const [modalOpen, setModalOpen] = useState(false);
  const [selectedInvalModal, setSelectedInvalModal] =
  ...
  >({});
  const [savingOperatorAttendance, setSavingOperatorAttendance] =
    useState(false);
  const [operatorSaveSuccess, setOperatorSaveSuccess] = useState<string | null>(
    null,
  );
  ...
  const fetchOperatorClassStudents = async (kelas: string, date: string) => {
    ...
    try {
      ...
      if (
        errSt &&
        (errSt.code === "42703" || errSt.message?.includes("academic_year"))
      ) {
  ...
  const toggleOperatorStudentStatus = (
    studentId: string,
    status: "S" | "I" | "A" | "D",
  ) => {
  ...
  const handleSaveOperatorAttendance = async () => {
    ...
    try {
      ...
      const inserts = Object.entries(operatorAttendance).map(
        ([studentId, status]) => ({
          date: filterDate,
  ...
  useEffect(() => {
    ...
    const attendanceChannel = supabase
      ...
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "homeroom_attendance" },
        () => {
          fetchMonitorData();
        },
      )
    ...
    try {
      ...
      const [
          ...
          .then(async (res) => {
            if (
              ...
              (res.error.code === "42703" ||
                res.error.message?.includes("academic_year") ||
                res.error.message?.includes("schedule_version"))
            ) {
              const fallback = await supabase
                .from("schedules")
                .select("*")
              ...
              if (fallback.error) {
                ...
                if (ultraFallback.data) {
                  ultraFallback.data = ultraFallback.data.filter(
                    (s) =>
        ...
        supabase
          .from("journals")
          .select(
            "teacher_id, kelas, subject, hours, cleanliness, validation, inval_teacher_name",
          )
          .gte("created_at", startOfDay)
          .lte("created_at", endOfDay)
          .then(async (res) => {
            if (
              res.error &&
              (res.error.code === "42703" ||
                res.error.message?.includes("validation") ||
                res.error.message?.includes("inval_teacher_name"))
            ) {
              return supabase
                .from("journals")
                .select("teacher_id, kelas, subject, hours, cleanliness")
                .gte("created_at", startOfDay)
                .lte("created_at", endOfDay);
            }
            return res;
        ...
        supabase
          .from("homeroom_attendance")
          .select("student_id, status, kelas, created_by")
          .eq("date", filterDate),
      ]);
      ...
      const processed: MonitorItem[] = schedules.map((sch) => {
        ...
        const matchingJournal = journals.find((j) => {
          ...
          const jHours = j.hours
            ? j.hours.split(",").map((h: string) => h.trim())
            : [];
        ...
        if (activeProfiles && activeProfiles.length > 0) {
          ...
          if (!p && sch.teacher_nip)
            p = activeProfiles.find((p) => p.nip === sch.teacher_nip);
        ...
        if (matchingJournal) {
          ...
          if (
            jVal === "inval" ||
            jInvalName ||
            (matchingJournal.teacher_id &&
              matchingJournal.teacher_id !== sch.teacher_id)
          ) {
            isInval = true;
            const subProfile = activeProfiles.find(
              (p) => p.id === matchingJournal.teacher_id,
            );
            substituteTeacherName = subProfile
              ? subProfile.full_name
              : "Guru Piket";
      ...
      const profileRoleMap: Record<
        string,
        { role?: string; nip?: string; name?: string }
      > = {};
      if (activeProfiles && activeProfiles.length > 0) {
      ...
      const uniqueAbsenceMap: Record<
        string,
        { name: string; status: string; kelas: string; source?: string }
      > = {};
      ...
      attendanceLogs.forEach((log: any) => {
        const isBambangPJOK =
          (log.teacher_name || "")
            .toLowerCase()
            .includes("bambang rastudianto") ||
          (log.subject || "").toLowerCase().includes("pendidikan jasmani") ||
      ...
      const absenceListFinal = Object.values(uniqueAbsenceMap).sort(
        (a, b) =>
          a.kelas.localeCompare(b.kelas) || a.name.localeCompare(b.name),
      );
      let sCount = 0,
        iCount = 0,
        aCount = 0,
      ...
      const kbmPct =
        totalJpRequired > 0
          ? Math.round((completedJp / totalJpRequired) * 1000) / 10
          : 0;

      const cleanCounts: Record<string, number> = {};
      journals.forEach((j) => {
        if (j.cleanliness === "sudah_bersih")
          cleanCounts[j.kelas] = (cleanCounts[j.kelas] || 0) + 1;
      });
      let cleanest = "-";
      let maxClean = -1;
      ...
      processed
        .filter((i) => !i.isFilled)
        .forEach((i) => {
          emptyCounts[i.kelas] = (emptyCounts[i.kelas] || 0) + 1;
        });
      let emptiest = "-";
      let maxEmpty = -1;
      ...
      setStats({
        alpaCount: absenceListFinal.length,
      ...
      const missing = processed
        .filter((i) => !i.isFilled)
        .map((i) => ({ name: i.teacherName, kelas: i.kelas }));
  ...
  const TableSection = ({
    ...
    return (
      <div className="flex flex-col h-full bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
        <div
          className={`py-2 px-3 text-white text-center font-extrabold uppercase tracking-wider ${colorClass} text-xs sm:text-sm`}
        >
          {title}
        </div>
        <div className="bg-white dark:bg-slate-900 flex-1 overflow-x-auto">
          <table className="w-full text-left table-fixed border-collapse">
            <thead className="bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 font-bold border-b border-slate-300 dark:border-slate-700 text-[11px]">
              <tr>
                <th className="px-0.5 py-1.5 w-[10%] text-center border-r border-slate-200 dark:border-slate-700">
                  Kelas
                </th>
                <th className="px-0.5 py-1.5 w-[9%] text-center border-r border-slate-200 dark:border-slate-700">
                  Jam
                </th>
                <th className="px-1.5 py-1.5 w-[66%] border-r border-slate-200 dark:border-slate-700">
                  Guru / Mapel
                </th>
            ...
            <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
              {displayItems.length === 0 ? (
                <tr>
                  <td
                    colSpan={4}
                    className="p-6 text-center text-slate-400 dark:text-slate-500 italic text-xs"
                  >
                    Tidak ada jadwal.
                  </td>
                </tr>
                ...
                displayItems.map((item) => (
                  <tr
                    key={item.scheduleId}
                    className={`transition-colors border-b border-slate-200 dark:border-slate-700 ${item.isFilled ? (item.isInval ? "bg-purple-50/40 dark:bg-purple-950/20 hover:bg-purple-100/50" : "bg-white dark:bg-slate-900 hover:bg-purple-50/50 dark:hover:bg-slate-800/60") : "bg-rose-50/70 dark:bg-rose-950/30 hover:bg-rose-100/50"}`}
                  >
                    <td className="px-0.5 py-1 text-center font-bold text-slate-800 dark:text-slate-200 text-xs border-r border-slate-200 dark:border-slate-700">
                      {item.kelas}
                    </td>
                    <td className="px-0.5 py-1 text-center font-mono font-medium text-slate-600 dark:text-slate-400 text-[11px] border-r border-slate-200 dark:border-slate-700">
                      {formatJam(item.jam)}
                    </td>
                    <td className="px-1.5 py-1 overflow-hidden border-r border-slate-200 dark:border-slate-700">
                      <div className="font-bold text-slate-800 dark:text-slate-100 truncate text-xs leading-snug">
                        {item.teacherName}
                      </div>
                      <div className="text-purple-700 dark:text-purple-400 font-semibold truncate text-[10px] leading-snug">
                        {item.mapel}
                      </div>
                    </td>
  ...
  const StatCard = ({
    ...
    >
      <div
        className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 ${bgClass} ${colorClass}`}
      >
        <Icon size={20} />
      </div>
      ...
      <div className="flex flex-col gap-4 pb-20">
              ...
              <div className="flex items-center gap-3 mt-0.5">
                <div className="flex items-center gap-1.5 bg-white px-2 py-0.5 rounded border border-slate-200 text-xs font-bold text-slate-600">
                  ...
                  {filterDate !== getWIBISOString() && (
                    <button
                      onClick={() => {
                        isManualDateRef.current = false;
                        setFilterDate(getWIBISOString());
                      }}
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="relative h-5 w-full overflow-hidden">
                {missingTeachers.length > 0 ? (
                  <div
                    className="absolute transition-all duration-500 ease-in-out transform w-full"
                    key={tickerIndex}
                  >
                    <p className="text-sm font-bold text-amber-800 truncate">
                      {missingTeachers[tickerIndex]?.name}{" "}
                      <span className="text-amber-600 font-normal text-xs">
                        ({missingTeachers[tickerIndex]?.kelas})
                      </span>
                    </p>
                  </div>
                ) : (
                  <p className="text-sm font-bold text-green-600">
                    Semua Guru Sudah Mengisi! 🎉
                  </p>
                )}
              </div>
            </div>
          ...
          <div className="flex items-center gap-2">
            <div className="relative">
              ...
              <input
                ...
                className="pl-8 pr-3 py-1.5 bg-white border border-slate-200 rounded-xl text-xs focus:ring-2 focus:ring-purple-500 w-40"
                value={searchTerm}
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          ...
          <StatCard
            ...
            value={stats.kbmPercentage}
            icon={Percent}
            colorClass="text-purple-600"
            bgClass="bg-purple-50"
          />
          <StatCard
            label="Kelas Terbersih"
            value={stats.cleanestClass}
            icon={Sparkles}
            colorClass="text-green-600"
            bgClass="bg-green-50"
          />
          <StatCard
            label="Jam Kosong Max"
            value={stats.mostEmptyClass}
            icon={Clock}
            colorClass="text-purple-600"
            bgClass="bg-purple-50"
          />
              ...
              <div className="flex flex-row md:flex-col items-center md:items-start gap-4 flex-shrink-0 md:min-w-[220px] md:border-r md:border-slate-100 dark:md:border-slate-700 md:pr-4 pt-2 md:pt-0">
                ...
                <div className="flex-1 w-full">
                  <h3 className="font-extrabold text-slate-800 dark:text-white text-xs uppercase tracking-wide leading-relaxed">
                    Rekap Absensi Kelas <br className="hidden md:block" />{" "}
                    (Absen Berkelompok)
                  </h3>
                  <p className="text-[10px] text-slate-500 dark:text-slate-400 mt-1 font-medium leading-tight">
                    Level Operator: Bebas pilih kelas untuk kunci absensi murid
                    di jurnal KBM.
                  </p>

                  <button
                    onClick={() =>
                      setShowOperatorInputForm(!showOperatorInputForm)
                    }
                  ...
                  <div className="p-4 border-2 border-purple-100 dark:border-slate-600 rounded-2xl bg-white dark:bg-slate-800 animate-fade-in shadow-sm space-y-4">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-2 border-b border-slate-100 dark:border-slate-700">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        <select
                          ...
                          onChange={(e) =>
                            setSelectedOperatorClass(e.target.value)
                          }
                    ...
                    <div className="flex justify-between items-center pt-2">
                      <span className="text-[11px] text-slate-500 dark:text-slate-400 italic">
                        {Object.keys(operatorAttendance).length} murid ditandai
                        tidak hadir.
                      </span>
                      <button
                ...
                <div className="flex items-center justify-between bg-slate-50 dark:bg-slate-700/50 p-2.5 rounded-xl border border-slate-100 dark:border-slate-700">
                  <div className="flex items-center gap-2 text-slate-500 dark:text-slate-400">
                    ...
                    <span className="text-[10px] font-bold uppercase tracking-wide">
                      Tanggal Rekap: {formatDateIndo(filterDate)}
                    </span>
                  </div>
        ...
        {modalOpen && (
          <div
            className="fixed inset-0 z-[9999] flex items-start justify-center pt-[calc(env(safe-area-inset-top)+1rem)] sm:p-4 bg-slate-900/50 backdrop-blur-sm transition-all duration-300"
            onClick={() => setModalOpen(false)}
          >
            <div
              className="bg-white rounded-2xl shadow-2xl w-full md:w-full md:max-w-md flex flex-col max-h-[85vh] overflow-hidden border border-slate-100 relative animate-fade-in"
              onClick={(e) => e.stopPropagation()}
            >
              <div className="flex justify-between items-center px-6 py-5 border-b border-gray-100">
                <div>
                  <h3 className="font-extrabold text-slate-800 text-lg leading-tight">
                    Rincian Ketidakhadiran Hari Ini
                  </h3>
              ...
              <div className="overflow-y-auto p-6 space-y-6 custom-scrollbar bg-white flex-1">
                <div className="grid grid-cols-4 gap-2">
                  ...
                  >
                    <span className="text-yellow-700 font-bold text-[10px] uppercase mb-0.5">
                      Sakit
                    </span>
                  ...
                  >
                    <span className="text-blue-700 font-bold text-[10px] uppercase mb-0.5">
                      Izin
                    </span>
                  ...
                  >
                    <span className="text-purple-700 font-bold text-[10px] uppercase mb-0.5">
                      Dispen
                    </span>
                  ...
                  >
                    <span className="text-red-700 font-bold text-[10px] uppercase mb-0.5">
                      Alpa
                    </span>
                ...
                <div className="p-3 bg-[#F9F7FF] border border-slate-100 rounded-xl text-center">
                  <span className="text-[10px] text-purple-500 font-bold uppercase">
                    *Termasuk input dari Wali Kelas & Guru Mapel.
                  </span>
                </div>
                    ...
                    <div className="flex justify-between items-center pb-2 border-b border-slate-200">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-extrabold text-xs text-slate-800 uppercase">
                          ...
                          {
                            absenceList.filter(
                              (s) => s.status === selectedStatusFilter,
                            ).length
                          }{" "}
                          Anak)
                        </span>
                      ...
                      {absenceList.filter(
                        ...
                        absenceList
                          ...
                          .map((student, idx) => (
                            <div
                              key={idx}
                              className="flex justify-between items-center bg-white p-2.5 rounded-xl border border-slate-100 text-xs shadow-sm"
                            >
                              <div className="flex items-center gap-2">
                  ...
                  <div className="space-y-3">
                    {allUniqueClasses.length === 0 ? (
                      ...
                      allUniqueClasses.map((cls) => {
                        ...
                        const presentCount =
                          totalStudents > 0 ? totalStudents - absentCount : 0;
                        const isExpanded = expandedClass === cls;
                        const hasAbsence = absentCount > 0;
                        return (
                          <div
                            key={cls}
                            className="border border-gray-100 rounded-2xl overflow-hidden transition-all hover:shadow-sm"
                          >
                            <button
                              onClick={() =>
                                hasAbsence &&
                            ...
                            >
                              <div className="flex items-center gap-3">
                                <div
                                  className={`w-10 h-10 flex items-center justify-center rounded-xl font-bold text-sm shadow-sm ${hasAbsence ? "bg-red-50 border border-red-100 text-red-700" : "bg-green-50 border border-green-100 text-green-700"}`}
                                >
                                  {cls}
                                </div>
                            ...
                            {isExpanded && hasAbsence && (
                              <div className="bg-gray-50 p-3 border-t border-gray-100 space-y-2 animate-fade-in">
                                {studentsInClass.map((s: any, idx: number) => (
                                  <div
                                    key={idx}
                                    className="flex justify-between items-center bg-white p-3 rounded-xl border border-gray-100 text-xs shadow-sm"
                                  >
                                    ...
                                    <div className="flex items-center gap-1.5">
                                      ...
                                      <span
                                        className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${s.status === "S" ? "bg-yellow-100 text-yellow-800" : s.status === "I" ? "bg-blue-100 text-blue-800" : s.status === "D" ? "bg-purple-100 text-purple-800" : "bg-red-100 text-red-800"}`}
                                      >
                                        {s.status === "S"
                                          ? "SAKIT"
                                          : s.status === "I"
        ...
        {selectedInvalModal && (
          ...
          >
            <div
              className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden border border-slate-100 dark:border-slate-700 relative animate-fade-in"
              onClick={(e) => e.stopPropagation()}
            >
              ...
              <div className="p-5 space-y-4 text-slate-700 dark:text-slate-200">
                <div className="bg-purple-50 dark:bg-purple-900/30 p-3.5 rounded-xl border border-purple-200 dark:border-purple-800 space-y-2.5 text-xs">
                  <div className="flex justify-between items-center border-b border-purple-100 dark:border-purple-800/60 pb-2">
                    ...
                    <span className="font-extrabold text-purple-800 dark:text-purple-200">
                      {selectedInvalModal.kelas} (Jam ke-
                      {formatJam(selectedInvalModal.jam)})
                    </span>
                  </div>
                  <div className="flex justify-between items-center border-b border-purple-100 dark:border-purple-800/60 pb-2">
                    <span className="font-semibold text-slate-500 dark:text-slate-400">
                  ...
                  <div className="flex justify-between items-center border-b border-purple-100 dark:border-purple-800/60 pb-2">
                    ...
                    <span className="font-bold text-rose-600 dark:text-rose-400">
                      {selectedInvalModal.teacherName}
                    </span>
                  </div>
                  <div className="flex justify-between items-center pt-0.5">
                    <span className="font-semibold text-slate-500 dark:text-slate-400">
                      Guru Pengganti / Piket:
                    </span>
                ...
                <p className="text-[11px] text-slate-500 dark:text-slate-400 text-center italic">
                  Jurnal KBM kelas ini diisi oleh guru pengganti secara
                  realtime.
                </p>
                <button
                  onClick={() => setSelectedInvalModal(null)}
