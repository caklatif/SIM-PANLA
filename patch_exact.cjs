...
const fullscreenBlockStart = "                    {/* Right (5 Cols): Realtime Activity History Feed in Fullscreen */}";
const fullscreenBlockEnd = "                        )}\n                      </div>\n                    </div>";

const startF = code.indexOf(fullscreenBlockStart);
const endF = code.indexOf(fullscreenBlockEnd, startF);

if (startF !== -1 && endF !== -1) {
    const replacementF = `                    {/* Right (5 Cols): Local Notification Panel */}
    ...
    code = code.substring(0, startF) + replacementF + code.substring(endF + fullscreenBlockEnd.length);
    console.log("Fullscreen block replaced.");
} else {
    console.log("Failed to find Fullscreen block start or end.");
}

// Now replace the regular block
// Find where it starts
const regularStartStr = "                {/* Right Last Scanned Card & Quick History (5 Cols) */}";
// Find where the regular block ends - which is before "              </div>\n            )}\n          </div>\n        )}\n      </main>"
const startR = code.indexOf(regularStartStr);
let endR = code.indexOf("                </div>\n              </div>\n            )}\n          </div>\n        )}\n      </main>");
