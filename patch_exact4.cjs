...
if (startR !== -1) {
    const endR = code.indexOf("              </div>\n            )}\n          </div>\n        )}\n\n        {/* TAB 2: KELOLA HASIL SCAN & DATABASE PRESENSI */}", startR);
    if (endR !== -1) {
        // Find the last </div> before this:
        const boundary = code.lastIndexOf("                </div>\n              </div>\n            )}\n", endR);
        code = code.substring(0, startR) + replacementR + code.substring(boundary);
        console.log("Regular block replaced.");
        fs.writeFileSync('pages/PresensiQR.tsx', code);
    } else {
        console.log("endR not found");
    }
} else {
    console.log("startR not found");
}
